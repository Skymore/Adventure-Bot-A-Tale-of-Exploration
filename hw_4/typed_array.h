...
class TypedArray {
   public:
    ...
    // operator+
    TypedArray<ElementType> operator+(const TypedArray<ElementType>& other);

    template <typename T>
    friend std::ostream& operator<<(std::ostream& os, TypedArray<T>& array);
...
template <typename ElementType>
TypedArray<ElementType>& TypedArray<ElementType>::operator=(
    const TypedArray<ElementType>& other) {
    if (this != &other) {
...
ElementType TypedArray<ElementType>::pop() {
    if (size() == 0) {
        throw std::range_error("Cannot pop from empty array");
    }
    ElementType x = get(size() - 1);
    end--;
    return x;
}
...
// concat
template <typename ElementType>
TypedArray<ElementType> TypedArray<ElementType>::concat(
    const TypedArray<ElementType>& other) {
    TypedArray<ElementType> result;
    for (int i = 0; i < size(); i++) {
        result.push(safe_get(i));
    }
    for (int i = 0; i < other.size(); i++) {
